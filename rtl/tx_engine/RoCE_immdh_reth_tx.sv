...
module RoCE_immdh_reth_tx  #(
    ...
    parameter CYCLE_IMMDH_COUNT = (HDR_IMMDH_SIZE+BYTE_LANES-1)/BYTE_LANES;

    parameter PTR_WIDTH = $clog2(CYCLE_IMMDH_COUNT);

    parameter OFFSET_IMMDH = HDR_IMMDH_SIZE % BYTE_LANES;
...
*/
    ...
    // 2 --> send immdh shifted payload
    reg [1:0] send_roce_payload_reg = 2'd0, send_roce_payload_next; // 0 = don't send, 1 = send payload as is, 2 = send payload shifted for immdh;
    ...
    reg [DATA_WIDTH-1 : 0] shift_roce_payload_32_axis_tdata;
    reg [KEEP_WIDTH-1 : 0] shift_roce_payload_32_axis_tkeep;
    reg                    shift_roce_payload_32_axis_tvalid;
    reg                    shift_roce_payload_32_axis_tlast;
    reg                    shift_roce_payload_32_axis_tuser;
    reg                    shift_roce_payload_32_axis_input_tready;
    reg                    shift_roce_payload_32_extra_cycle_reg = 1'b0;
    ...
    wire append_immdh = s_roce_bth_op_code == RC_RDMA_WRITE_LAST_IMD
    ...
    // RETH
    always @* begin
        if (OFFSET_IMMDH == 0) begin
            // passthrough if no overlap
            shift_roce_payload_32_axis_tdata  = s_roce_payload_axis_tdata;
            shift_roce_payload_32_axis_tkeep  = s_roce_payload_axis_tkeep;
            shift_roce_payload_32_axis_tvalid = s_roce_payload_axis_tvalid;
            shift_roce_payload_32_axis_tlast  = s_roce_payload_axis_tlast;
            shift_roce_payload_32_axis_tuser  = s_roce_payload_axis_tuser;
            shift_roce_payload_32_axis_input_tready = 1'b1;
        end else if (shift_roce_payload_32_extra_cycle_reg) begin
            shift_roce_payload_32_axis_tdata = {s_roce_payload_axis_tdata, save_roce_payload_axis_tdata_reg} >> ((KEEP_WIDTH-OFFSET_IMMDH)*8);
            shift_roce_payload_32_axis_tkeep = {{KEEP_WIDTH{1'b0}}       , save_roce_payload_axis_tkeep_reg} >> (KEEP_WIDTH-OFFSET_IMMDH);
            shift_roce_payload_32_axis_tvalid = 1'b1;
            shift_roce_payload_32_axis_tlast = save_roce_payload_axis_tlast_reg;
            shift_roce_payload_32_axis_tuser = save_roce_payload_axis_tuser_reg;
            shift_roce_payload_32_axis_input_tready = flush_save;
        end else begin
            shift_roce_payload_32_axis_tdata = {s_roce_payload_axis_tdata, save_roce_payload_axis_tdata_reg} >> ((KEEP_WIDTH-OFFSET_IMMDH)*8);
            shift_roce_payload_32_axis_tkeep = {s_roce_payload_axis_tkeep, save_roce_payload_axis_tkeep_reg} >> (KEEP_WIDTH-OFFSET_IMMDH);
            shift_roce_payload_32_axis_tvalid = s_roce_payload_axis_tvalid;
            shift_roce_payload_32_axis_tlast = (s_roce_payload_axis_tlast && ((s_roce_payload_axis_tkeep & ({KEEP_WIDTH{1'b1}} << (KEEP_WIDTH-OFFSET_IMMDH))) == 0));
            shift_roce_payload_32_axis_tuser = (s_roce_payload_axis_tuser && ((s_roce_payload_axis_tkeep & ({KEEP_WIDTH{1'b1}} << (KEEP_WIDTH-OFFSET_IMMDH))) == 0));
            shift_roce_payload_32_axis_input_tready = !(s_roce_payload_axis_tlast && s_roce_payload_axis_tready && s_roce_payload_axis_tvalid);
        end
    end
    ...
    always @* begin
        ...
        end else if (send_roce_payload_reg == 2'd2) begin // send IMMDH shifted payload
            s_roce_payload_axis_tready_next = m_roce_bth_payload_axis_tready_int_early && shift_roce_payload_32_axis_input_tready;

            m_roce_bth_payload_axis_tdata_int = shift_roce_payload_32_axis_tdata;
            m_roce_bth_payload_axis_tkeep_int = shift_roce_payload_32_axis_tkeep;
            m_roce_bth_payload_axis_tlast_int = shift_roce_payload_32_axis_tlast;
            m_roce_bth_payload_axis_tuser_int = shift_roce_payload_32_axis_tuser;

            if ((s_roce_payload_axis_tready && s_roce_payload_axis_tvalid) || (m_roce_bth_payload_axis_tready_int_reg && shift_roce_payload_32_extra_cycle_reg)) begin
                transfer_in_save = 1'b1;

                m_roce_bth_payload_axis_tvalid_int = 1'b1;

                if (shift_roce_payload_32_axis_tlast) begin
                    flush_save = 1'b1;
                    s_roce_payload_axis_tready_next = 1'b0;
                    ptr_next = 0;
                    send_roce_payload_next = 2'd0;
        ...
        if (m_roce_bth_payload_axis_tready_int_reg && (!OFFSET_IMMDH || (send_roce_payload_reg == 2'd0) || m_roce_bth_payload_axis_tvalid_int)) begin
            if (send_roce_header_reg) begin
                ptr_next = ptr_reg + 1;

                if ((OFFSET_IMMDH != 0) && (CYCLE_IMMDH_COUNT == 1 || ptr_next == CYCLE_IMMDH_COUNT-1) && (send_roce_payload_reg == 2'd0)) begin
                ...
                `_HEADER_RETH_FIELD_(0, roce_immdh_data_reg[3*8 +: 8])
                `_HEADER_RETH_FIELD_(1, roce_immdh_data_reg[2*8 +: 8])
                `_HEADER_RETH_FIELD_(2, roce_immdh_data_reg[1*8 +: 8])
                `_HEADER_RETH_FIELD_(3, roce_immdh_data_reg[0*8 +: 8])

                if (ptr_reg == (HDR_IMMDH_SIZE-1)/BYTE_LANES) begin
    ...
    always @(posedge clk) begin
        ...
        if (store_roce_hdrs) begin
            ...
            if (s_roce_bth_valid && append_immdh) begin
        ...
        if (flush_save) begin
            ...
            shift_roce_payload_32_extra_cycle_reg <= 1'b0;
        end else if (transfer_in_save) begin
            save_roce_payload_axis_tlast_reg <= s_roce_payload_axis_tlast;
            shift_roce_payload_32_extra_cycle_reg <= OFFSET_IMMDH ? s_roce_payload_axis_tlast && ((s_roce_payload_axis_tkeep & ({KEEP_WIDTH{1'b1}} << (KEEP_WIDTH-OFFSET_IMMDH))) != 0) : 1'b0;
        end
