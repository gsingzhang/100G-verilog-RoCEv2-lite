...
module RoCE_bth_udp_tx  #(
    ...
    parameter CYCLE_BTH_COUNT = (HDR_BTH_SIZE+BYTE_LANES-1)/BYTE_LANES;

    parameter PTR_WIDTH = $clog2(CYCLE_BTH_COUNT);

    parameter OFFSET_BTH = HDR_BTH_SIZE % BYTE_LANES;
...
*/
    ...
    reg send_roce_payload_reg = 1'b0, send_roce_payload_next;
    ...
    always @* begin
        if (OFFSET_BTH == 0) begin
            ...
            shift_roce_payload_96_axis_tdata = {s_roce_payload_axis_tdata, save_roce_payload_axis_tdata_reg} >> ((KEEP_WIDTH-OFFSET_BTH)*8);
            shift_roce_payload_96_axis_tkeep = {{KEEP_WIDTH{1'b0}}       , save_roce_payload_axis_tkeep_reg} >> (KEEP_WIDTH-OFFSET_BTH);
            ...
            shift_roce_payload_96_axis_tdata = {s_roce_payload_axis_tdata, save_roce_payload_axis_tdata_reg} >> ((KEEP_WIDTH-OFFSET_BTH)*8);
            shift_roce_payload_96_axis_tkeep = {s_roce_payload_axis_tkeep, save_roce_payload_axis_tkeep_reg} >> (KEEP_WIDTH-OFFSET_BTH);
            shift_roce_payload_96_axis_tvalid = s_roce_payload_axis_tvalid;
            shift_roce_payload_96_axis_tlast = (s_roce_payload_axis_tlast && ((s_roce_payload_axis_tkeep & ({KEEP_WIDTH{1'b1}} << (KEEP_WIDTH-OFFSET_BTH))) == 0));
            shift_roce_payload_96_axis_tuser = (s_roce_payload_axis_tuser && ((s_roce_payload_axis_tkeep & ({KEEP_WIDTH{1'b1}} << (KEEP_WIDTH-OFFSET_BTH))) == 0));
    ...
    always @* begin
        ...
        if (s_roce_bth_ready && s_roce_bth_valid) begin
            ...
            send_roce_payload_next = (OFFSET_BTH != 0) && (CYCLE_BTH_COUNT == 1);
            s_roce_payload_axis_tready_next = (send_roce_payload_next == 1'b1) && m_udp_payload_axis_tready_int_early;
            
        end

        if (send_roce_payload_reg) begin
            ...
            if ((s_roce_payload_axis_tready && s_roce_payload_axis_tvalid) || (m_udp_payload_axis_tready_int_reg && shift_roce_payload_96_extra_cycle_reg)) begin
                ...
                if (shift_roce_payload_96_axis_tlast) begin
                    ...
                    s_roce_payload_axis_tready_next = 1'b0;
                    ptr_next = 0;
                    send_roce_payload_next = 1'b0;
                end
            end
        end


        if (m_udp_payload_axis_tready_int_reg && (!OFFSET_BTH || (send_roce_payload_reg == 1'b0) || m_udp_payload_axis_tvalid_int)) begin
            if (send_roce_header_reg) begin
                ptr_next = ptr_reg + 1;

                if ((OFFSET_BTH != 0) && (CYCLE_BTH_COUNT == 1 || ptr_next == CYCLE_BTH_COUNT-1) && (send_roce_payload_reg == 1'b0)) begin
                    send_roce_payload_next = 1'b1;
                    s_roce_payload_axis_tready_next = m_udp_payload_axis_tready_int_early && shift_roce_payload_96_axis_input_tready;
                end
                ...
                if (ptr_reg == (HDR_BTH_SIZE-1)/BYTE_LANES) begin
                    if (send_roce_payload_reg == 1'b0) begin
        ...
        s_roce_bth_ready_next = !m_udp_hdr_valid_next && !((send_roce_header_next != 1'b0) || (send_roce_payload_next != 1'b0));
    end
    ...
    always @(posedge clk) begin
        ...
        busy_reg <= (send_roce_header_next != 1'b0) || (send_roce_payload_next != 1'b0);
        ...
        if (flush_save) begin
            ...
            shift_roce_payload_96_extra_cycle_reg <= OFFSET_BTH ? s_roce_payload_axis_tlast && ((s_roce_payload_axis_tkeep & ({KEEP_WIDTH{1'b1}} << (KEEP_WIDTH-OFFSET_BTH))) != 0) : 1'b0;
            end
